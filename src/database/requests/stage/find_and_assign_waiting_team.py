...
async def find_and_assign_waiting_team(
        game_id: UUID,
        actor_id: UUID
):
        ...
        for team in teams:
            ...
            already = await session.scalar(
                select(func.count()).select_from(StageModel).where(
                    StageModel.team_id == team.id, StageModel.actor_id == actor_id
                )
            )
            if (already or 0) > 0:
                continue

            visited_sq = select(StageModel.actor_id).where(StageModel.team_id == team.id)
