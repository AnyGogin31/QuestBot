...
async def get_game_stats(game_id: UUID):
    async with database_session() as session:
        total_teams = await session.scalar(
            select(func.count()).select_from(TeamModel).where(TeamModel.game_id == game_id)
        ) or 0
        finished_teams = await session.scalar(
            select(func.count()).select_from(TeamModel).where(
                TeamModel.game_id == game_id, TeamModel.status == TeamStatus.FINISHED
            )
        ) or 0
        free_actors = await session.scalar(
            select(func.count()).select_from(ActorModel).where(
                ActorModel.game_id == game_id, ActorModel.status == ActorStatus.FREE
            )
        ) or 0
        busy_actors = await session.scalar(
            select(func.count()).select_from(ActorModel).where(
                ActorModel.game_id == game_id, ActorModel.status == ActorStatus.BUSY
            )
        ) or 0
        total_stages = await session.scalar(
            select(func.count()).select_from(StageModel).where(StageModel.game_id == game_id)
        ) or 0
        done_stages = await session.scalar(
            select(func.count()).select_from(StageModel).where(
                StageModel.game_id == game_id, StageModel.status == StageStatus.COMPLETED
            )
        ) or 0
    return {
