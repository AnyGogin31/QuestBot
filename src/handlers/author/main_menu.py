...
async def my_games(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not games:
        await safe_edit(callback, "У вас пока нет игр", author_main())
        return
    await safe_edit(callback, "📋 <b>Ваши игры:</b>", games_list(games))
...
async def back_main(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await safe_edit(callback, "👋 <b>QuestBot</b>\n\nВыберите действие:", author_main())
